...
\subsection{The geometric sign flip: Compression vs.\ maintenance}

A key result of this work is the identification of a \emph{geometric sign flip} in the thermodynamic cost of dimensionality reduction. The work required to \emph{form} a low-dimensional representation and the power required to \emph{maintain} it arise from distinct dynamical regimes, and manifold curvature contributes with opposite sign in each.

During the \emph{compression phase}---the transition $\rho_0 \rightarrow \rho_M$ that maps an initial distribution onto a target manifold $M$---positive curvature induces geodesic convergence. Trajectories naturally focus toward the manifold, effectively amplifying the gradient descent flow. Consequently, the geometric contribution to the compression work, $W_{\mathrm{geom}}$, can be \emph{negative} relative to a flat baseline, as the manifold geometry itself assists in ``funneling'' probability mass into the lower-dimensional state:
\begin{equation}
W_{\mathrm{geom}} \propto -\int_M \kappa_{\mathrm{eff}}(x)\,\|\nabla\phi(x)\|^2\,d\mu_M(x),
\end{equation}
where $\kappa_{\mathrm{eff}}$ is an effective curvature proxy and the negative sign reflects geometric focusing.

In contrast, during the \emph{maintenance phase}, when the system must remain localized on $M$ against stochastic perturbations, curvature produces the opposite effect. Increased curvature expands the set of noise directions leading off the manifold, increasing the effective escape volume. To maintain a fixed stationary variance on a curved manifold, stronger restoring forces are required, generating higher maintenance power:
\begin{equation}
P_{\mathrm{keep}} \propto D\int_M \kappa_{\mathrm{eff}}(x)\, d\mu_M(x),
\end{equation}
where $D$ is the diffusion coefficient.

Thus curvature that reduces the cost of \emph{forming} the representation simultaneously increases the cost of \emph{holding} it:
\begin{center}
\emph{High curvature: easy to write, hard to hold.}\\
\emph{Low curvature: harder to write, cheap to maintain.}
\end{center}

This sign flip establishes a geometric law for information-processing manifolds. It suggests an architectural design principle for biological, neural, and neuromorphic systems: dynamically modulate curvature---increase it to facilitate rapid state transitions; decrease it to minimize metabolic rent during retention. The same geometry that accelerates collapse imposes a cost on stability, revealing a structural tension at the heart of dissipative information encoding.
